...
- [x] Task 2.2: `ComponentGenerator.Layout.adjust_density_constraints` を使用して、サンプリングされた `mu` が物理制約を遵守するように調整する。
- [x] Task 2.3: 代表パターン（均一密度、中央集中等）を初期サンプルとして生成する機能を追加する。
  - _Requirement:_ 1.2
...
- [x] Task 3.4: 解析結果を JLD2 に保存する際、メタデータとしてユニークなスナップショットIDと調整済み `mu` を含める。また、成功した結果を `data/raw/` へ移動しマニフェストを更新する。

## 統合と検証
- [x] Task 4.1: `SnapshotGenerator.jl` エントリポイントを実装し、一連のフローを統合する。
- [x] Task 4.2: ユニットテスト: Sampler の分布と制約遵守を検証する。
- [x] Task 4.3: E2Eテスト: 少数のサンプルで `mu -> 展開 -> FVM -> JLD2保存 -> Manifest更新` が完走することを確認する。
