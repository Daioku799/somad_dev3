...
TSVの配置領域およびバリデーションにおける境界基準を、チップ全体ではなく、実際にTSVが配置されるシリコン層の領域（マージン 0.1mm を除いた領域）に限定し、物理ソルバー（FVM）が要求する厳密な幾何形状オブジェクトを生成することを目的とする。

## Boundary Context
- **In scope**:
  - シリコン実装領域（チップ境界から 0.1mm 内側）に基づく密度マップセル分割とTSV本数計算。
  - シリコン実装領域内での幾何学的なTSV座標展開（中央寄せ格子配置）。
  - TSV座標に同期したはんだバンプの自動生成。
  - シリコン実装領域および最小ピッチに対する物理的制約の検証（境界違反および干渉のチェック）。
- **Out of scope**:
  - `geometry-logic` が担当する幾何学的な内外判定（点判定）。
  - `model-builder` が担当するIDマップへの具体的な埋め込み処理。
  - 熱物性値の管理。
  - GDSIIファイルからのシリコン形状ロード（本スケジュールでは対象外）。
- **Adjacent expectations**:
  - `config-loader` から有効なチップ寸法、TSV半径、最小ピッチ、および密度マップ `mu` が提供されること。

## Requirements

### Requirement 1: 密度マップに基づくTSV本数計算
...
5. The Component Generator shall 合計TSV本数が物理的制限 $N_{limit}$ を超えないように $\mu$ をスケーリングする共有ユーティリティ関数 `adjust_density_constraints(mu, n_max_matrix, N_limit)` を提供する。

### Requirement 2: セル内での実TSV座標展開
**Objective:** 解析システムとして、密度値からFVMで使用可能な具体的な (x, y) 座標を生成したい。

#### Acceptance Criteria
1. Where `tsv_mode` が "density" の場合、The Component Generator shall チップ外周から `0.1e-3` (0.1mm) のマージンを差し引いたシリコン実装領域（`[0.1e-3, lx - 0.1e-3] × [0.1e-3, ly - 0.1e-3]`）を $G_x \times G_y$ に等分割し、各セル内で算出された $n_{ij}$ 本のTSVを、指定されたレイアウト規則（既定：セル内中央寄せ格子配置）に従って配置する。
...
### Requirement 3: はんだバンプの自動生成
**Objective:** 解析システムとして、TSV의 配置に合わせて接続部のはんだバンプを自動的に生成したい。
...
### Requirement 4: 物理的制約の検証
**Objective:** システム管理者として、解析実行前に物理的に不可能な配置（干渉や境界逸脱）を検出したい。

#### Acceptance Criteria
1. If 生成されたTSVまたはバンプの物理的な範囲（中心座標と半径を含む）がシリコン実装領域（`[0.1e-3, lx - 0.1e-3] × [0.1e-3, ly - 0.1e-3]`）を逸脱する場合、The Component Generator shall 境界違反エラーを通知し、処理を中断する。
