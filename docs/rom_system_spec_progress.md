...
* **特徴 (修正前/修正後の対比)**:
  左側のIDマップにおいて、修正前（旧仕様）はTSV（黄）がチップ境界の限界位置まで配置されているのに対し、修正後（新仕様）はマージン `0.1mm` を考慮して内側に寄せて配置されています。
...
* **特徴 (修正前/修正後の対比)**:
  中央領域にTSVが集中する物理形状です。こちらも境界側のマージン制限による配置領域の切り替わりが確認できます。
...
* **再構成・予測結果の比較 (修正前/修正後の対比)**:
  シリコン制限の有無により、三次元的な境界対流効果の強さが変化するため、FVM数値解とROM予測解の誤差挙動にも明確な変化が見られます。
...
* **物理的な考察**:
  ...
  * 修正前と修正後を比較すると、シリコンマージン `0.1mm` を設けて境界効果を緩和した修正後の方が、温度分布全体がより滑らかになり、ROMの予測誤差（Tmax誤差など）がさらに抑制される効果（1.215 K ➔ 1.180 Kへの改善）が実証されています。
...
*   **特徴**: L2相対誤差は `0.129%`、Tmax絶対誤差は僅か **`0.303 K`** となり、実用上完全に一致する予測精度を示しています。
...
*   **特徴**: 中央部と外周部が高度に対称に揃った特徴的なパターンであり、今回の学習データ（26枚）がカバーする主空間から外れた**「外挿（extrapolation）」状態**になったため、RBF予測が過小評価気味（温度場が低め）になり、Tmax絶対誤差が **`2.646 K`** （合格基準 2.0 K を超過）に達しました。
...
* **RBFモデルのハイパーパラメータ最適化**:
  * 外挿の境界領域における予測精度（FAILケースの解消）を向上させるため、RBF補間時のカーネルパラメータ `epsilon` や正則化定数 `lambda` のチューニング、またはさらなるデータ拡張を実行します。
* **GA最適化モジュール (`ga-optimizer`) の開発・結合**:
  * 今回構築・検証された「密度マップ ➔ FVM ➔ ROM」の予測パイプラインを用い、遺伝的アルゴリズム (GA) によってホットスポット温度を最小化するTSV最適配置パターンを探索するモジュールの結合設計に入ります。
