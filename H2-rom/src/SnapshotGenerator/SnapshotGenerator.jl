...
export run_generator, run_density_generator

"""
    run_density_generator(n_cases::Int; gx=4, gy=4, radius=5e-6, solver_dir="../H2-main-ext", data_dir="../data")

Main entry point for generating snapshots using density maps.
"""
function run_density_generator(n_cases::Int; gx=4, gy=4, radius=5e-6, solver_dir="../H2-main-ext", data_dir="../data")
    ...
    println("Generating $n_cases new density samples using LHS...")
    new_params = generate_density_samples(n_cases, gx=gx, gy=gy, radius=radius)
    ...
    # 4. Run loop
    run_batch!(manifest, pending_ids, solver_dir, work_dir, raw_dir, manifest_path)
...
Helper function to run a batch of cases.
"""
function run_batch!(manifest, pending_ids, solver_dir, work_dir, raw_dir, manifest_path)
